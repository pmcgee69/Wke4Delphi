...
// "cookie格式必须是:Set-cookie: PRODUCTINFO=webxpress; domain=.fidelity.com; path=/; secure") \
  ...
  mbOnClose: procedure(webView: TmbWebView; callback: mbDestroyCallback; param: Pointer); stdcall;
  ...
  mbRunJs: procedure(webView: TmbWebView; frameId: TmbWebFrameHandle; const script: string; isInClosure: boolean;
...
begin
  ...
  begin
    OutputDebugString(PChar(mbFile));
    if Fileexists(mbFile) then
    begin
      mbLibFileName := mbFile;
      mbLibHandle := GetModuleHandle(PChar(ExtractFileName(mbLibFileName)));
    end;
    if (mbLibHandle = 0) and FileExists(mbLibFileName) then
    begin
      mbLibHandle := LoadLibrary(PChar(mbLibFileName));
     // SetCurrentDirectory(PChar(ExtractFilePath(mbLibFileName )));
      AddPathEnvironment(ExtractFilePath(mbLibFileName));
...
//  mbSetHeadlessEnabled := GetprocAddress(mbLibHandle, 'mbSetHeadlessEnabled');
 ...
 // if not Assigned(mbCanGoForward ) then  showmessage('mbCanGoForward');
  ...
  mbOnNetGetFavion := GetprocAddress(mbLibHandle, 'mbOnNetGetFavion');
